...
public class AdminPresentationTest {
    ...
    static CustomerManagementPanel customers(){repository.InMemoryUserRepository r=new repository.InMemoryUserRepository();model.User u=new model.User(999,"admin","admin@example.com","unused",model.UserRole.ADMIN,true,model.AccountStatus.ACTIVE);r.save(u);return new CustomerManagementPanel(new service.AuthService(r),u);}
    public static void main(String[] args)throws Exception{SwingUtilities.invokeAndWait(()->{try{
        render(new StaffApprovalsPanel(),"staff");render(customers(),"customers");
