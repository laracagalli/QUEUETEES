...
public class SignupFrame extends JFrame implements ActionListener {
        ...
        private final JFormattedTextField birthday;
        ...
        public SignupFrame(AuthService authService) {
                ...
                int totalFrames = bgGif.getIconWidth();
                float[] alpha = { 1.0f };
                ...
                birthday = new JFormattedTextField(dateMask);
                ...
                birthday.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
                        @Override
        ...
        private void handleRegistration() {
                ...
                String birthdayText = birthday.getText().replace("_", "").trim();

                if (birthdayText.length() < 10) {
                        showWarning("Please enter a complete birthday (MM/DD/YYYY).");
                        return;
                }

                // Birthday format validation (MM/DD/YYYY)
                try {
                        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MM/dd/yyyy");
                        parsedBirthday = LocalDate.parse(birthday.getText(), formatter);

                        // Limit the maximum date to 12/31/2026
                        LocalDate maxDateLimit = LocalDate.of(2026, 12, 31);
                        if (parsedBirthday.isAfter(maxDateLimit)) {
                                showWarning("Birthday cannot be later than 12/31/2026.");
                                return;
                        }

                } catch (DateTimeParseException ex) {
                        showWarning("Birthday must use MM/DD/YYYY.\nExample: 12/22/2000.");
                        return;
                }
                ...
                new EmailAuthFrame(
                                authService,
                                result.getUser()).setVisible(true);
        }
